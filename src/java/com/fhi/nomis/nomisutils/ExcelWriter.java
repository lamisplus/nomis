...
public WritableWorkbook writeOvcDataFromListOfIndicatorsToExcelForUSAID(String indicatorName,OutputStream os, List ovcList) 
{
        ...
        try 
        {
            ...
            String ageCategory="";
            String cgiverHivStatus=null;  
            String cgiverCurrentHivStatus=null;
            String cgiverCurrentStatus=null;
            ...
            for (int i = 0; i < ovcList.size(); i++) 
            {
                ...
                if(cgiver !=null)
                {
                    ...
                    cgiverHivStatus=cgiver.getCgiverHivStatus();
                    cgiverCurrentHivStatus=cgiver.getActiveHivStatus().getHivStatus();
                    if(cgiverCurrentHivStatus==null || cgiverCurrentHivStatus.trim().length()==0 || cgiverCurrentHivStatus.trim().equalsIgnoreCase("unknown") || cgiverCurrentHivStatus.trim().equalsIgnoreCase("select"))
                    {
                        if(cgiverHivStatus !=null && cgiverHivStatus.trim().length()>0 && (cgiverHivStatus.trim().equalsIgnoreCase("negative") || cgiverHivStatus.trim().equalsIgnoreCase("positive")))
                        cgiverCurrentHivStatus=cgiverHivStatus;
                    }
                    cgiverCurrentStatus=cgiver.getReasonForWithdrawal();
                }
                ...
                if(cgiverCurrentHivStatus !=null && cgiverCurrentHivStatus.equalsIgnoreCase("positive"))
...
public WritableWorkbook writeOvcDataFromListOfIndicatorsToExcel(String indicatorName,OutputStream os, List ovcList) 
{
        ...
        try 
        {
            ...
            wworkbook = Workbook.createWorkbook(os);
            wsheet = wworkbook.createSheet(sheetName + (sheetCount + 1), sheetCount);
            String[] columnHeadings = {"State", "Lga", "CBO", "Ward/Community", "Indicator","HH unique Id", "Caregiver Id","Caregiver Baseline HIV status","Caregiver Current HIV status","Caregiver current status","OVC Id", "Day of Enrollment","Month of Enrollment","Year of Enrollment", "Age at baseline", "Age unit", "Current age", "Current age unit","Sex" ,"Eligibility criteria","Baseline Hiv status","Current HIV Status","Date of current Hiv status (yyyy-mm-dd)","In Care","On ART","Facility enrolled","Point of update","Birth Registration status","Risk assessed","At Risk of HIV","Date of last assessment","Baseline school status","Current school status","Household headship","Baseline Household vulnerability status","Current Household vulnerability status","OVC Baseline CSI","OVC Baseline Vulnerability status","OVC Current CSI score","OVC Current CSI status", "OVC Status","Date of current status","HH withdrawn from Program","Reason HH withdrawn","Date of withdrawal","Age category","No. Of OVC"};
            ...
            for (int i = 0; i < ovcList.size(); i++) 
            {
                ...
                wsheet.addCell(label);
                label = new Label(++cellCount, row, cgiverCurrentHivStatus);
                wsheet.addCell(label);
