...
public class LoadUpInfo implements Serializable
{
    ...
    public List getUserAssignedLgas(User user,String stateCode)
    {
        ...
        if(user !=null)
        {
            if(user.getUserroleId().equalsIgnoreCase(NomisConstant.SUPERUSER_ROLEID) || user.getUserGroupId().equalsIgnoreCase(NomisConstant.DEFAULTGRPID) || user.getUserGroupId().equalsIgnoreCase("defaultgpId"))
            lgaList=getLgasPerStateFromOrgRecords(stateCode);
            else
            {
